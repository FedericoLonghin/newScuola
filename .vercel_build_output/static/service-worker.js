...
async function b(e, t) {
	...
	const a = e.clone(),
		n = { headers: new Headers(a.headers), status: a.status, statusText: a.statusText },
		r = t ? t(n) : n,
		i = (function () {
			if (void 0 === v) {
			...
			f = d
				? await (async function (e, t, s, a) {
						...
						const r = o(c({}, a), { ignoreSearch: !0 }),
							i = await e.keys(t, r);
						for (const c of i) if (n === R(c.url, s)) return e.match(c, a);
				  })(p, n.clone(), ['__WB_REVISION__'], u)
				: null;
		try {
	...
	async getCacheKey(e, t) {
		if (!this._cacheKeys[t]) {
			...
			for (const e of this.iterateCallbacks('cacheKeyWillBeUsed'))
				s = L(await e({ mode: t, request: s, event: this.event, params: this.params }));
			this._cacheKeys[t] = s;
		}
...
class k {
	...
	handleAll(e) {
		...
		const t = e.event,
			...
			n = new q(this, { event: t, request: s, params: a }),
	...
	async _awaitComplete(e, t, s, a) {
		...
		if (
			(await t.runCallbacks('handlerDidComplete', { event: a, request: s, response: n, error: r }),
			t.destroy(),
			r)
		)
			throw r;
...
class K extends k {
	...
	async _handleFetch(e, t) {
		...
		if (!this._fallbackToNetwork)
			throw new l('missing-precache-entry', { cacheName: this.cacheName, url: e.url });
	...
	_useDefaultCacheabilityPluginIfNeeded() {
		...
		for (const [s, a] of this.plugins.entries())
			a !== K.copyRedirectedCacheableResponsesPlugin &&
				(a === K.defaultPrecacheCacheabilityPlugin && (e = s), a.cacheWillUpdate && t++);
		0 === t
...
(K.defaultPrecacheCacheabilityPlugin = {
	...
	(K.copyRedirectedCacheableResponsesPlugin = {
		cacheWillUpdate: async ({ response: e }) => (e.redirected ? await b(e) : e)
	});
class T {
		...
		return g(e, async () => {
			...
			for (const [n, r] of this._urlsToCacheKeys) {
				const t = this._cacheKeysToIntegrities.get(r),
					...
					a = new Request(n, { integrity: t, cache: s, credentials: 'same-origin' });
				await Promise.all(
	...
	createHandlerBoundToURL(e) {
		...
		return (s) => (
			...
			(s.params = c({ cacheKey: t }, s.params)),
...
function S(e, t, s) {
	...
	if ('string' == typeof e) {
		...
		a = new j(({ url: e }) => e.href === n.href, t, s);
	} else if (e instanceof RegExp) a = new O(e, t, s);
	else if ('function' == typeof e) a = new j(e, t, s);
	else {
		if (!(e instanceof j))
	...
	return (W || ((W = new M()), W.addFetchListener(), W.addCacheListener()), W).registerRoute(a), a;
}
class E extends j {
	constructor(e, t) {
		super(({ request: s }) => {
...
const D = ['/', '/scuola'];
I(D.map((e) => ({ url: e, revision: '1636219785735' })));
S(
